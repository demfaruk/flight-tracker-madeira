...
# ── Loaded from GitHub Secrets (you set these up next) ─────────────────────
...
# ── Fetch price calendar from Skyscanner via RapidAPI ───────────────────────
def fetch_calendar(depart_from, depart_to, return_from, return_to):
...
# ── Find 10 cheapest trips for a given stay duration ────────────────────────
def get_cheapest_trips(duration):
    today      = datetime.now()
    ...
    # Navigate response — try two common structures
    days = None
    try:
        ...
        try:
            ...
            print(f"[{duration}n] Unexpected response: {list(data.keys())}")
            return []
...
def build_email(results):
    ...
    <p style="color:#666;">Daily report &mdash; {today_str} &nbsp;|&nbsp; Direct flights &nbsp;|&nbsp; Round trip &nbsp;|&nbsp; Prices in EUR</p>
    <hr/>"""
    ...
    for duration, trips in results.items():
        ...
        for i, t in enumerate(trips, 1):
            ...
            html += f"""<tr style="border-bottom:1px solid #ddd;">
                ...
                <td style="padding:8px;"><strong>&euro;{t['price']:.0f}</strong></td>
            </tr>"""

        html += "</table>"

    html += """<br/><p style="color:#aaa;font-size:11px;">
        Prices from Skyscanner via RapidAPI. Verify before booking.</p>
    </body></html>"""
    return html
...
def main():
    ...
    for d in DURATIONS:
        ...
        results[d] = get_cheapest_trips(d)
